...
# 存储结果列表（简化）
pre_errors_v1 <- numeric(100)  # 预分配空间提升性能
...
for (iter in 1:100) {
  ...
  # 2. 准备训练数据（移除批次划分）
  train_x <- as.matrix(v1_sub_data[, 1:3])
  train_y <- as.numeric(v1_sub_data[[4]])
...
# 保持原有保存逻辑
file_name <- paste0("100tvae_", task_id, ".csv")
