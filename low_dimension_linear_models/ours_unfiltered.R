...
read_target_data <- function(file_path) {
  ...
  target_x <- as.matrix(target_data[, 1:3])
  target_y <- as.numeric(target_data[[4]])
...
seq1 <- seq(from = 0.01, to = 0.2, by = 0.01)

# 生成从 0.3 到 1，步长为 0.1 的序列
seq2 <- seq(from = 0.3, to = 1, by = 0.1)
# 合并两个序列
...
for (iter in 1:100) {
  ...
  create_batches <- function(data, batch_size) {
    lapply(seq(1, nrow(data), by = batch_size), function(j) {
      ...
      source_x <- as.matrix(data[j:end_row, 1:3, drop = FALSE])
      source_y <- as.numeric(unlist(data[j:end_row, 4]))
  ...
  fit_gaussian_v1 <- glmtrans(D_training_v1$target, D_training_v1$source, transfer.source.id= "all", cores = 1)
  ...
  fit_gaussian_v2 <- glmtrans(D_training_v2$target, D_training_v2$source,transfer.source.id= "all", cores = 1)
...
if (length(pre_errors_v1) > 0 && length(pre_errors_v2) > 0) {
  ...
  avg_len_v1v2 <- (avg_len_v1 + avg_len_v2) * 100
  ...
  # 保存为 CSV 文件
  file_name <- paste0("100avg_final_results_run_", task_id, ".csv")
