...
for (iter in 1:100) {
  ...
  fit_binomial_v1 <- glmtrans(D_training_v1$target, D_training_v1$source, family = "binomial", cores = 1, C0 = 2, detection.info = FALSE, transfer.source.id = "all")
  ...
  fit_binomial_v2 <- glmtrans(D_training_v2$target, D_training_v2$source, family = "binomial", cores = 1, C0 = 2, detection.info = FALSE, transfer.source.id = "all")
  ...
  combined_eta <- cbind(1, test_data$x) %*% combined_beta
...
if (length(pre_errors_v1_v2_combined) > 0) {
  avg_pre_error_v1_v2_combined <- mean(pre_errors_v1_v2_combined)
  
  # 保存为 CSV 文件
  file_name <- paste0("highlogall_", task_id, ".csv")
  write.csv(data.frame(avg_pre_error_v1_v2_combined = avg_pre_error_v1_v2_combined), file_name, row.names = FALSE)
}
