...
# Lasso回归
fit.lasso <- cv.glmnet(x = v1_v2_x_known, y = v1_v2_y_known,family = "binomial",type.measure = "class")
y.pred.lasso <- predict(fit.lasso, test_data$x,type= "class")
pre_error_before <- mean((as.numeric(y.pred.lasso) - test_data$y)^2)
...
seq3 <- seq(from = 0.6, to = 1, by = 0.1)
# 合并两个序列
split_ratios <- c(seq1, seq2,seq3)
# 选择当前任务的切分比例
...
for (iter in 1:100) {
  ...
  fit_binomial_v1 <- glmtrans(D_training_v1$target, D_training_v1$source, family = "binomial", cores = 1,C0=2)
  v1_transferable_sources <- fit_binomial_v1$transfer.source.id
  ...
  fit_binomial_v2 <- glmtrans(D_training_v2$target, D_training_v2$source,family = "binomial", cores = 1,C0=2)
  v2_transferable_sources <- fit_binomial_v2$transfer.source.id
  
  if (length(v2_transferable_sources) == 0) next
  
  pre_error_v1_generate_glmtrans <- mean((as.numeric(predict(fit_binomial_v1, test_data$x,type = "class")) != test_data$y))
  v1_estimate_error <- mean((fit_binomial_v1$beta - beta_0)^2)
  pre_errors_v1 <- c(pre_errors_v1, pre_error_v1_generate_glmtrans)
  estimate_errors_v1 <- c(estimate_errors_v1, v1_estimate_error)
  betas_v1 <- c(betas_v1, list(fit_binomial_v1$beta))
  len_v1 <- c(len_v1, length(v1_transferable_sources))
  

  
  pre_error_v2_generate_glmtrans <- mean((as.numeric(predict(fit_binomial_v2, test_data$x,type= "class")) != test_data$y))
  v2_estimate_error <- mean(fit_binomial_v2$beta - beta_0)^2
  pre_errors_v2 <- c(pre_errors_v2, pre_error_v2_generate_glmtrans)
  estimate_errors_v2 <- c(estimate_errors_v2, v2_estimate_error)
  betas_v2 <- c(betas_v2, list(fit_binomial_v2$beta))
  len_v2 <- c(len_v2, length(v2_transferable_sources))
  
  # 计算组合误差
  combined_beta <- (fit_binomial_v1$beta + fit_binomial_v2$beta) / 2
  combined_eta <- test_1x %*% combined_beta
  combined_prob <- sigmoid(combined_eta)
  combined_prediction <- as.numeric(combined_prob > 0.5)
  pre_error_v1_v2_combined_iter <- mean(combined_prediction != test_data$y)
...
if (length(pre_errors_v1) > 0 && length(pre_errors_v2) > 0) {
  ...
  avg_len_v1v2 <- (avg_len_v1 + avg_len_v2)/2 * batch_size
  ...
  final_results <- list(
    ...
    pre_error_before = 0.29661
  )
  
  # 保存为 CSV 文件
  file_name <- paste0("n2-200_", task_id, ".csv")
